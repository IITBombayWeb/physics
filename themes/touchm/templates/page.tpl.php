...
drupal_add_js('jQuery(document).ready(function(){
	...
	jQuery("input:checked").parent().addClass("opendiv");
	
 	var region1wrap=jQuery(".container.region1wrap").height();
	var region2wrap=jQuery(".container.region2wrap").height();
	var region3wrap=jQuery(".container.region3wrap").height();
	var table_height= region1wrap+region2wrap+region3wrap;
 jQuery(window).scroll(function(){
				    var y = jQuery(document).scrollTop(),

        fixedmenu = jQuery(".block.block-menu-block");

    if (y >= table_height) {

        fixedmenu.addClass("fixed_menu");

    } else {

        fixedmenu.removeClass("fixed_menu");

    }
 });
	
 jQuery(".mainslider-container.responsive li,.view.view-research-landing-page table.views-view-grid td").click(function() { 
	var cbox=jQuery(this).find("a");
	 window.open(cbox.attr("href"),"_parent");
